...
client = GitHubClient()
release_manager = ReleaseManager(client, appConfig)
...
def main(force_update=False):
    ...
    # update for changed OAM-repos (or changed implementation) only
    delta = timedelta(hours=4, minutes=45)
    oam_updated = OamRepo.get_updated_within(oam_repos, delta)
    ...
    oam_hardware_raw, oam_stat = release_manager.process_releases(oam_releases_data)
    ...
    with open(os.path.join("data", 'ofms.json'), 'r', encoding='utf-8') as f:
        ofm_data = {
            ofm_object.full_name: ofm_object
            for ofm_object in [OfmData.from_dict(ofm) for ofm in json.load(f)]
        }
